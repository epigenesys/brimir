...
RSpec.describe(AttachmentsController, :type => :controller) do
  ...
  describe 'attaching jpegs' do
    before do
      ...
      attachment.update!(:file => fixture_file_upload(Rails.root.join('spec', 'fixtures', 'attachments', 'default-testpage.jpg'), "image/jpeg"))
    end
    ...
    it("should show thumb") do
      skip "There seems to be significant issues with the thumbnail generation, but Paperclip will need to be replaced with a more modern solution so we'll work around it in the short-term."
  ...
  describe 'attaching PDFs' do
    before do
      sign_in(charlie)
      Tenant.current_domain = Tenant.first.domain
      attachment.update!(:file => fixture_file_upload(Rails.root.join('spec', 'fixtures', 'attachments', 'default-testpage.pdf'), "application/pdf"))
    end
